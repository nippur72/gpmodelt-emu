...
class AsciiKeyboard {
   emulator: GPEmulator;            // reference to emulator for reading its clock
   key_pressed_ascii: number;       // the actual ASCII code of the key pressed
   key_pressed_tick: number;        // keep track of cpu cycle when they key was pressed

   constructor(emulator: GPEmulator) {
      this.emulator = emulator;         // reference emulator for reading its clock
      this.key_pressed_ascii = 0;       // no key pressed so far
      this.key_pressed_tick = -1000000; // set last keypress beyond start of time
   ...
   keyboard_presskey(keyascii: number) {
      this.key_pressed_ascii = keyascii;
      this.key_pressed_tick = this.emulator.cycles;  // remember the CPU cycle the key was pressed
   }

   // called from cpu I/O
   keyboard_read(): number {
      const strobe_duration = this.emulator.cpuSpeed/100; // 1/100 sec
      const strobe = this.emulator.cycles < (this.key_pressed_tick + strobe_duration) ? 1 : 0;
      let data = (this.key_pressed_ascii & 0x7f) | (strobe << 7);
      return ~data & 0xFF; // negated logic
   }

   browserKeyDown(e: KeyboardEvent) {
      ...
      */
      ...
      if(k !== 0) {
         // disable autorepeat as in the original GP Ascii keyboard
         if(!e.repeat) {
   ...
   paste(text: string) {
      for(let t=0;t<text.length;t++) {
         ...
         this.keyboard_presskey(v);
         this.emulator.renderAllLines();
         this.emulator.renderAllLines();
         this.emulator.renderAllLines();
         this.emulator.renderAllLines();
      }
