class Audio {
   ...
   constructor(bufsize: number) {
      ...
      this.playing = false;
      this.buffers = [];
      this.audioContext = new window.AudioContext(); //new (window.AudioContext || window.webkitAudioContext)();
   ...
   playBuffer(buffer: Float32Array) {
      ...
      this.buffers.push(new Float32Array(buffer));  // push a cloned copy
   }
