...
export const SASI_HDC_MEDIA_SIZE = 256 * 256 * 32 * 4;
export const SASI_FDC_MEDIA_SIZE =  32 * 256 * 77 * 2;

class HardDisk {
   image: Uint8Array;

   constructor(image: Uint8Array | undefined, size: number) {
      this.image = image === undefined ? new Uint8Array(size).fill(0xE5) : this.resize(image, size);
   }

   resize(image: Uint8Array, size: number) {
...
class SASI_CONTROLLER {
   ...
   hard_disks: HardDisk[];     // the four hard disks

   sa_ticks: number;           // tick counter

   STATE: string = "UNKNOWN";

   DATA_IN: number = 0;        // data byte from CPU to SA
   DATA_OUT: number = 0;       // data byte from SA to CPU
   ...
   constructor() {
      ...
      this.hard_disks = [
         new HardDisk(undefined, SASI_HDC_MEDIA_SIZE),
         new HardDisk(undefined, SASI_FDC_MEDIA_SIZE),
         new HardDisk(undefined, SASI_HDC_MEDIA_SIZE),
         new HardDisk(undefined, SASI_HDC_MEDIA_SIZE)
      ];
   }

   lun_to_id(byte: number) {
   ...
   SASI_write_data(value: number): void {
      this.DATA_IN = value;
   ...
   SASI_write_pins(value: number): void {
...
export { HardDisk, SASI_CONTROLLER };
