// TODO make a true monochromatic palette

import { GPEmulator } from "./emulator";

import { rom_GCAR_A_U7 } from "./roms/rom.GCAR_A_U7";
import { rom_GCAR_B_U8 } from "./roms/rom.GCAR_B_87";
import { rom_GCE_M1_U3 } from "./roms/rom.GCE_M1_U3";
import { rom_GCE_M2_U4 } from "./roms/rom.GCE_M2_U4";
...
class VideoRenderer {
   ...
   bmp: Uint32Array | undefined;

   SCREEN_COLUMNS: number;
   SCREEN_ROWS: number;
   SCREEN_COLUMNS_ARR: number;

   TEXT_W: number;
   TEXT_H: number;

   BORDER_V: number;
   BORDER_V_BOTTOM: number;
   BORDER_H: number;

   DOUBLE_SCANLINES: number;
   ...
   calculateGeometry() {
      ...
      if(!this.canvas) throw "unable to find canvas";
      ...
      if(!this.canvasContext) throw "unable to get context";
   ...
   drawFrame_y()
   {
      ...
      if(this.raster_y >= this.SCREEN_H) {  // TODO numscanlines
         this.raster_y = 0; 
         this.canvasContext!.putImageData(this.imageData!, 0, 0);  // TODO remove "!"
      }
   }
   
   drawFrame_y_border(y: number)
   {
      // draw borders
   ...
   drawFrame_y_text(y: number) {
      ...
      {
         let code = this.emulator.memory[this.video_ram+(row*this.SCREEN_COLUMNS_ARR)+x];
         ...
         for(let xx=0;xx<8;xx++) {
            ...
            if(!reverse) pixel_color = (bitmap & (128>>xx)) > 0 ? fg : bg;
   ...
   setPixelBorder(x: number, y: number, color: number) {
   ...
   setPixel640(x: number, y: number, color: number) {
      const c0 = this.palette.palette[color];   
...
class Palette {
   ...
   halfpalette = new Uint32Array(16);
   saturation = 1.0;
   ...
   applySaturation(r: number, g: number, b: number, s: number) {
   ...
   setPalette(i: number, r: number, g: number, b: number) {
