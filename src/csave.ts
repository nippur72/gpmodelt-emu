...
class Tape {
   ...
   tapeSampleRate: number;
   tapeBuffer: Float32Array;
   tapeLen: number;
   tapePtr:number;
   tapeHighPtr: number;

   csaveBufferSize: number;
   csaveBuffer: Float32Array | undefined;
   csavePtr: number;
   csaveDownSampleCounter: number;
   csaving: boolean;
   ...
   constructor(emulator: GPEmulator) {
      ...
      this.csavePtr = 0;                     // points to the write position in the csaveo buffer
      this.csaveDownSampleCounter = 0;       // counter used to downsample from CPU speed to 48 Khz
   ...
   cloadAudioSamples(n: number) {
      ...
      if(this.tapeHighPtr >= this.emulator.cpuSpeed) {
         this.tapeHighPtr-= this.emulator.cpuSpeed;
         this.emulator.cassette_bit_in = this.tapeBuffer[this.tapePtr] > 0 ? 1 : 0;
         this.tapePtr++;      
      }
   }   

   cload(buffer: Float32Array, sampleRate: number) {
   ...
   cstop() {
      ...
      const buffer = WavEncoder.encode.sync(wavData, { bitDepth: 16, float: false, symmetric: false });
