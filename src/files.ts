/*
async function load(filename: string, p: number) {
...
async function save(filename: string, p1?: number, p2?: number) {
   const ext = getFileExtension(filename);
...
function loadBytes(bytes: number[], address: number, fileName: string) {
