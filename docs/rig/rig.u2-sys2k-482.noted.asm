CURRDRIVE       EQU 0BFDCh
CDRIVE_ALT      EQU 0BB87h      ; alternate current drive ?
DPH_TABLE       EQU 0BA6Bh
DSK_BUF         EQU 0BBACh      ; disk sector buffer
DSKBUF_PTR      EQU 0BFEAh      ; disk sector buffer pointer
DISK_UNIT_TABLE EQU 0BA61h      ; disknumber to disk unit word table
UNIT_WORD       EQU 0BB82h      ; disk unit word
...
E4C6: 2A EA BF                          LD      HL,(DSKBUF_PTR)
E4C9: E5                                PUSH    HL
E4CA: 21 00 FE                   LE4CA: LD      HL,0FE00h
E4CD: 22 EA BF                          LD      (DSKBUF_PTR),HL
E4D0: CD 12 E8                          CALL    0E812h
E4D3: E1                         LE4D3: POP     HL
E4D4: 22 EA BF                          LD      (DSKBUF_PTR),HL
...
E50A: C9                                RET
                                 ...
                                 RW_PREPARE:
E50B: 3A 87 BB                          LD      A,(CDRIVE_ALT)
E50E: FD 21 82 BB                       LD      IY,UNIT_WORD
E512: 21 AC BB                          LD      HL,DSK_BUF
E515: 22 EA BF                          LD      (DSKBUF_PTR),HL
                                        ; --- START PROC LE518 ---
E518: 21 61 BA                   LE518: LD      HL,DISK_UNIT_TABLE
E51B: 5F                                LD      E,A
E51C: 16 00                             LD      D,00h
E51E: 19                                ADD     HL,DE
E51F: 19                                ADD     HL,DE
...
E521: 23                                INC     HL
E522: 56                                LD      D,(HL)
E523: ED 53 82 BB                       LD      (UNIT_WORD),DE
E527: C9                                RET
...
; *************************************************************
; READ - CP/M bios entry
; *************************************************************
                                 READ:
E697: CD 0B E5                          CALL    RW_PREPARE
E69A: AF                                XOR     A
...
E76B: 3A 87 BB                   LE76B: LD      A,(CDRIVE_ALT)
...
E788: 11 AC BB                   LE788: LD      DE,DSK_BUF
...
references to external address UNIT_WORD:
        E50E LD IY,UNIT_WORD
        E523 LD (UNIT_WORD),DE

references to external address CDRIVE_ALT:
        E50B LD A,(CDRIVE_ALT)
        E65F LD (CDRIVE_ALT),A
        E6C3 LD A,(CDRIVE_ALT)
        E6E0 LD A,(CDRIVE_ALT)
        E747 LD A,(CDRIVE_ALT)
        E76B LD A,(CDRIVE_ALT)
...
references to external address DSKBUF_PTR:
        E4C6 LD HL,(DSKBUF_PTR)
        E4CD LD (DSKBUF_PTR),HL
        E4D4 LD (DSKBUF_PTR),HL
        E515 LD (DSKBUF_PTR),HL
...
possible references to external address DISK_UNIT_TABLE:
        E518 LD HL,DISK_UNIT_TABLE

possible references to external address DPH_TABLE:
        E672 LD DE,DPH_TABLE

possible references to external address UNIT_WORD:
        E50E LD IY,UNIT_WORD
        ----------
        E523 LD (UNIT_WORD),DE
...
possible references to external address DSK_BUF:
        E512 LD HL,DSK_BUF
        E593 LD HL,DSK_BUF
        E788 LD DE,DSK_BUF
