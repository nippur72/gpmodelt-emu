...
VERIFY1:LD	A,(DE)
	CP	(HL)
	JP	NZ,XALT		;jump to XALT routine.
	INC	DE
	INC	HL
	DEC	B
	JP	NZ,VERIFY1
...
;
XALT:	LD	HL,76F3H	;'DI HLT' instructions.
	LD	(CBASE),HL
	LD	HL,CBASE
	JP	(HL)
;
...
CKCONSOL: LD	A,(CHARBUF)	;check buffer.
	...
	JP	NZ,CKCON1
	CALL	CONIN		;XALT processing until another char
...
;
BITMAAP:	LD	HL,(DSKSIZE)	;compute size of allocation table.
	LD	C,3
	CALL	SHIFTR		;(HL)=(HL)/8.
...
LOGINDRV: LD	HL,(LOGIN)	;get the login vector.
	...
	JP	BITMAAP		;now update the bitmap.
;
